...
class ColeccionistaDetailTest {
    ...
    fun testDetailColeccionista(){
        ...
        onView(withId(R.id.collectorRv)).perform(actionOnItemAtPosition<RecyclerView.ViewHolder>(0, click()))
        Thread.sleep(3000)
        onView(withId(R.id.textViewCollectorName)).check(matches(withText(containsString("Manolo Bellon"))))
        onView(withId(R.id.scrollAlbumList)).check(matches(ViewMatchers.isDisplayed()))
        onView(withId(R.id.scrollPerformerList)).check(matches(ViewMatchers.isDisplayed()))
    }
}
